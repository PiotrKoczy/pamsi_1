...
int main()
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        result = "wyn_quicksort_fullrng_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] / 4);
        result = "wyn_quicksort_25_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] / 2);
        result = "wyn_quicksort_50_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] * 0.75);
        result = "wyn_quicksort_75_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);
        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] * 0.95);
        result = "wyn_quicksort_95_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] * 0.99);
        result = "wyn_quicksort_99_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        quickSort(testArray, size_array[a] * 0.997);
        result = "wyn_quicksort_99_7_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);

        copyArray(testArray, randomArray, size_array[a]);
        reverseArray(testArray, size_array[a]);
        result = "wyn_quicksort_reverse_" + std::to_string(size_array[a]) + ".csv";
        test(testArray, size_array[a], result, i);
    }
    }
    return 0;
